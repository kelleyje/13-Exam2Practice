...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.space = self.volume - len(self.contents)
    ...
    def append_string(self, additional_contents):
        ...
        if len(additional_contents) > self.space:
            for x in range(self.space):
                self.contents += additional_contents[x]

            for x in range(len(additional_contents) - self.space):
                note += additional_contents[x + self.space]
    ...
    def shrink(self, new_volume):
        ...
        if self.space < 0:
            ...
            for x in range(self.volume):
                self.contents += mono[x]

            for x in range(len(mono) - self.volume):
                note += mono[x + self.volume]
    ...
    def steal(self, other_box):
        ...
        appendage, other_box.contents = other_box.contents[0: self.space], other_box.contents[self.space: len(other_box.contents)]
        self.append_string(appendage)
